...
const AddNew = () => {
    ...
    const publishPost = async () => {
        try {
            const res = await fetch(ApiConfig.api_url + '/articles', {
                method: 'POST',
                body: JSON.stringify({
                    title,
                    category,
        ...
        <Main>
            ...
            <form action="">
                <div className='mb-3'>
                    ...
                    <input name="title" id='title' type="text" placeholder="Title" className="focus:shadow-primary-outline text-sm leading-5.6 ease block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 outline-none transition-all placeholder:text-gray-500 focus:border-blue-500 focus:outline-none" onChange={handleTitle} />
                </div>
                <div className='mb-3'>
                    <label htmlFor="category" className="text-sm leading-5 font-medium text-gray-700 mb-2">Category</label>
                    <input name="category" id='category' type="text" placeholder="Category" className="focus:shadow-primary-outline text-sm leading-5.6 ease block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 outline-none transition-all placeholder:text-gray-500 focus:border-blue-500 focus:outline-none" onChange={handleCategory} />
                </div>
                <div className='mb-3'>
                    <label htmlFor="content" className="text-sm leading-5 font-medium text-gray-700 mb-2">Content</label>
                    <textarea name="content" id='content' rows={10} placeholder="Content" className="focus:shadow-primary-outline text-sm leading-5.6 ease block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 outline-none transition-all placeholder:text-gray-500 focus:border-blue-500 focus:outline-none" onChange={handleContent} />
                </div>
                <button type="button" className="mr-3 inline-block px-6 py-3 font-bold text-center bg-gradient-to-tl from-blue-500 to-violet-500 uppercase align-middle transition-all rounded-lg cursor-pointer leading-normal text-xs ease-in tracking-tight-rem shadow-xs bg-150 bg-x-25 hover:-translate-y-px active:opacity-85 hover:shadow-md text-white" onClick={publishPost}>
                    Publish
                </button>
                <button type="button" className="mr-3 inline-block px-6 py-3 font-bold text-center bg-gradient-to-tl from-orange-500 to-yellow-500 uppercase align-middle transition-all rounded-lg cursor-pointer leading-normal text-xs ease-in tracking-tight-rem shadow-xs bg-150 bg-x-25 hover:-translate-y-px active:opacity-85 hover:shadow-md text-white" onClick={draftPost}>Draft</button>
